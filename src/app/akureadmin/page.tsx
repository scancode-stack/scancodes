...
export default function DashboardPage() {
  ...
  const fetchFoods = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_AUTH_API_URL}/food/all-food?page=${page}&limit=${limit}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const confirmDeleteFood = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_AUTH_API_URL}/food/delete-food`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleVatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_AUTH_API_URL}/food/edit-food/${vatFoodId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
